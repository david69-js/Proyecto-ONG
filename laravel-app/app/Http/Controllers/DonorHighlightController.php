...
{
    /**
     * Mostrar listado de donadores destacados
     */
    public function index(Request $request)
    {
        $donations = Donation::orderByDesc('id')->limit(50)->get();
    ...
    public function update(Request $request, DonorHighlight $highlight)
    {
        ...
        if ($request->hasFile('avatar')) {
            if ($highlight->avatar_path) {
                Storage::disk('public')->delete($highlight->avatar_path);
            }
            $data['avatar_path'] = $request->file('avatar')->store('donors/avatars', 'public');
        }
    ...
    /**
     * Alternar publicación (publicar / despublicar)
     */
